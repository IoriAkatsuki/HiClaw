...
ThreadSafeQueue<std::shared_ptr<std::vector<uint8_t>>> g_bs_queue(kVencQueueSize);
...
AclLiteError VencHelper::GetEncodedFrame(std::vector<uint8_t> &out)
{
    std::shared_ptr<std::vector<uint8_t>> buf = g_bs_queue.Pop();
    if (buf == nullptr) {
        return ACLLITE_ERROR; // queue empty
    }
    out = std::move(*buf);
    return ACLLITE_OK;
}
...
AclLiteError DvppVenc::SaveVencFile(void* vencData, uint32_t size)
{
    ...
    void* data = CopyDataToHost(vencData, size, vencInfo_.runMode, MEMORY_NORMAL);
    if (data == nullptr) {
        ACLLITE_LOG_ERROR("Copy venc data to host failed");
        return ACLLITE_ERROR_COPY_DATA;
    }

    if (!vencInfo_.outFile.empty()) {
        size_t ret = fwrite(data, 1, size, outFp_);
        if (ret != size) {
            ACLLITE_LOG_ERROR("Save venc file %s failed, need write %u bytes, but only write %zu bytes, error: %s",
...
AclLiteError DvppVenc::CreateInputPicDesc(ImageData& image)
{
    ...
    if (vencInfo_.runMode != ACL_DEVICE) {
        ...
        if (aclRet != ACL_SUCCESS) {
            ACLLITE_LOG_ERROR("acl memcpy data to dev failed, image.size=%u, ret=%d.\n", image.size, aclRet);
