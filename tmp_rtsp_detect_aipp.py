...
def postprocess(pred_flat, ratio, dwdh, nc=61, conf_thres=0.55, iou_thres=0.45, names=None):
    ch = 4 + nc
    ...
    cand = int(np.count_nonzero(mask))
...
def main():
    ...
    model = AclLiteModel(args.model); model.load(); model.context = res.context
    ...
    sock = None
    target = None
    # 如需 RTSP 推流再启用 DVPP，当前禁用以避免影响 ACL 上下文
    ...
    last_dump = 0
    last_ts = None
    dump_interval = 0.05  # WebUI 刷新间隔，20fps 级别
    ...
    try:
        while True:
            ...
            if outputs:
                # 记录原始最大类别分数，方便调阈值
                cls_max = float(np.max(outputs[0][4:]))
                dets, cand, kept = postprocess(outputs[0], ratio, (dw, dh), names=names)
            ...
            if now - last_dump > dump_interval:
                ...
                max_score = max([d['score'] for d in dets], default=0.0)
                state = {
                    'fps': fps,
                    'infer_ms': infer_ms,
                    'dets': dets[:50],
