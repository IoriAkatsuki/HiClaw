...
@st.cache_resource
def load_model():
    # Path to the downloaded Qwen2.5-7B model
    model_path = "/home/HwHiAiUser/ICT/qwen25_fastllm/models/qwen/Qwen/Qwen2.5-7B-Instruct"
    
    if not os.path.exists(model_path):
        st.error(f"No model found at {model_path}. Please run the download script first.")
        return None

    st.info(f"Loading model from: {model_path}")
    
    try:
        # Load using Transformers first
        tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True)
        hf_model = AutoModelForCausalLM.from_pretrained(model_path, trust_remote_code=True).eval()
        
        # Convert to FastLLM with float16 precision
